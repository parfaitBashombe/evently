...
import { FaCalendarCheck, FaSpinner, FaFloppyDisk } from "react-icons/fa6";
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogTrigger asChild>
        <button className="flex items-center gap-1.5 rounded-lg border border-white/10 px-3 py-1.5 text-xs font-semibold text-white/40 transition-colors hover:border-[#60a5fa]/30 hover:text-[#60a5fa]">
      ...
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto border-white/10 bg-[#0e1528] p-0" showCloseButton={false}>
        ...
        <DialogHeader className="flex flex-row items-center justify-between gap-3 px-6 pt-6 pb-4 border-b border-white/8">
          <DialogTitle className="flex items-center gap-2.5 text-base font-bold text-white">
            <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#004ac6]/15">
              <FaCalendarCheck className="h-4 w-4 text-[#60a5fa]" />
            </span>
            Edit Event
          </DialogTitle>
          <button
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-0">
          ...
          <div className="px-6 py-5 border-b border-white/8 flex flex-col gap-4">
            ...
            <Field>
              <FieldLabel htmlFor="ee-title">Title <span className="text-[#60a5fa]">*</span></FieldLabel>
            ...
            <Field>
              <FieldLabel htmlFor="ee-desc">Short description</FieldLabel>
              <textarea id="ee-desc" value={description} onChange={(e) => setDescription(e.target.value)}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field>
                <FieldLabel htmlFor="ee-cat">Category</FieldLabel>
                <div className="relative">
                  <select id="ee-cat" value={category} onChange={(e) => setCategory(e.target.value)} disabled={pending} className={selectClass}>
              ...
              <Field>
                <FieldLabel htmlFor="ee-loc">Location</FieldLabel>
                <input id="ee-loc" value={location} onChange={(e) => setLocation(e.target.value)}
          ...
          <div className="px-6 py-5 border-b border-white/8 flex flex-col gap-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field>
                <FieldLabel htmlFor="ee-start">Start date &amp; time</FieldLabel>
                <input id="ee-start" type="datetime-local" value={eventDate} onChange={(e) => setEventDate(e.target.value)} disabled={pending} className={inputClass} />
              </Field>
              <Field>
                <FieldLabel htmlFor="ee-end">End date &amp; time</FieldLabel>
                <input id="ee-end" type="datetime-local" value={endDate} onChange={(e) => setEndDate(e.target.value)} disabled={pending} className={inputClass} />
              </Field>
            </div>
            <Field>
              <FieldLabel htmlFor="ee-cap">Capacity</FieldLabel>
              <input id="ee-cap" type="number" min="1" value={capacity} onChange={(e) => setCapacity(e.target.value)}
          ...
          <div className="flex items-center justify-end gap-2 px-6 py-4 border-t border-white/8">
            ...
            <button type="submit" disabled={pending || !title.trim()}
              ...
              {pending ? (
                ...
                <><FaFloppyDisk className="h-3.5 w-3.5" /> Save changes</>
              )}
            </button>
          </div>
        </form>
